    ...
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (
          ...
          campsite.comments.author.equals(req.user._id)
        ) {
          if (req.body.rating) {
          ...
          err.status = 404;
          return next(err);
        } else if (!campsite.comments.author.equals(req.user._id)) {
          err = new Error('You cannot edit a comment made by somebody else!');
          err.status = 403;
          return next(err);
        } else {
          err = new Error(`Comment ${req.params.commentId} not found`);
    ...
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (
          ...
          err = new Error('You cannot delete comments made by another user.');
          err.status = 403;
          return next(err);
        } else {
          err = new Error(`Comment ${req.params.commentId} not found`);
